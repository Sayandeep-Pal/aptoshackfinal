...
export default function ManualDeploymentPanel({ generatedCode }: DeploymentPanelProps) {
    // State variables adapted from the App component and existing DeploymentPanel
    const [moveCode, setMoveCode] = useState( '// Initial value can be empty or set to generatedCode if provided');
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            {/* Code Editor */ }
            {/* <div className="p-4 border-b bg-white">
                <Label htmlFor="moveCode">Move Code:</Label>
                <Input
                    id="moveCode"
                    type="text"
                    value={moveCode}
                    onChange={(e) => setMoveCode(e.target.value)}
                    placeholder="Enter your Move code here"
                    className="mt-2 h-32"
                    disabled={loading}
                />
            </div> */}
                ...
                <div className="m-4 bg-white">
                    <CardContent className='h-full pb-4'>
                        <ScrollArea className="h-full">
                            <textarea
                                className="w-full h-96 p-2 border border-gray-300 rounded resize-y"
                                value={moveCode}
                                onChange={(e) => setMoveCode(e.target.value)}
                            />
                        </ScrollArea>
